...
import InputField, { InputType, makeDisplayList } from '../../0-Assets/field-sync/input-config-sync/inputField';
...
const FormInput = ({...props}:{key:any, validateUniqueFields?:boolean, FIELDS:InputField[], getInputField:(field:string) => any|undefined, setInputField:(field:string, value:any) => void, onSubmitText:string, onSubmitCallback:()=>void|Promise<void>, onAlternativeText?:string, onAlternativeCallback?:()=>void|Promise<void>, headerChildren?:ReactElement, footerChildren?:ReactElement}) => {
    ...
    const getSelectDisplayValue = (fieldField:string, value:string):string => {
        const field:InputField|undefined = props.FIELDS.find(f => f.field === fieldField);
    ...
    return (
        <form key={props.key} id={props.onSubmitText} >
            ...
            {
                props.FIELDS.filter(field => !field.hide).map((f) => 
