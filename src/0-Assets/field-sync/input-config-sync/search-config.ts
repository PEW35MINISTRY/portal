...
export enum SearchType {
  ...
  export class SearchTypeInfo {
    ...
    constructor(props:{ searchType:SearchType, displayTitle:string, itemType:ListItemTypesEnum, getID:(item:any) => number, route:string, 
        ...
        /* Validations */
        if(this.searchRefineList.includes.length > 0 && !this.searchRefineList.includes('ALL')) throw new Error(`search-config: Search Type:${this.searchType} does not include 'ALL' in searchRefineList.`);
    }
  }
